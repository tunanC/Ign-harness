...
**跨设备个人 AI —— 一个大脑住在你的 Server，每台设备都是它的身体。**

**任意设备、任何时间随叫随到；数据只留在你自己的 Server——不经第三方，官方也读不到。**

简体中文 | [English](./README.md)

一句话：一个 Server 当大脑，PC、眼镜、手机、机器人等当手脚。在任意一台设备上说一句话，Server 负责规划，把工具分发到对应设备执行，结果回传汇总。

- **聊天**：自然语言对话，支持多轮上下文
- **规划-执行（PE）引擎**：LLM 把任务拆成计划，逐项调用能力，汇总结果
- **跨设备路由**：工具分发到适合它的设备上跑（眼镜、PC、机器人等），不在 Server 上执行
- **声明式能力**：加一个文件夹 = 加一种能力
...
今天的 AI 被困在一个个产品里：聊天软件里的 AI 不认识你写的代码，编辑器里的 AI 不认识你的日程——它们互不相识，没有一个 AI 认识完整的你。而你身边的设备正在越来越多：手机、手表、眼镜、车、机器人……"每个设备里塞一个 AI"这条路注定崩坏。
...
- 自然语言对话（纯聊天，也支持多轮上下文）
- **规划-执行（PE）引擎**：LLM 把任务拆成计划，逐项调用能力，汇总结果
- **跨设备路由**：工具不在 Server 上执行，按设备类型分发到对应客户端（如眼镜、PC、机器人等），结果回传
...
3. **一份协议，两端执行**：同一套协议，在 Server 上读写文件，在眼镜上拍照，在 PC 上操作应用，在机器人身上执行动作——LLM 不必关心工具最终跑在哪。
...
- **为什么 Server 和 Client 分离？** 因为设备（眼镜/手机/车机/机器人等）算力有限，统一由一个 Server 思考，设备只负责执行和交互。
